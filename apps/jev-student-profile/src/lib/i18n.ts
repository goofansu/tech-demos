...
export const en = {
  ...
  app: {
    ...
    batchFooter:
      "Each row is one Jev {systemone} call with the full rubric, through {evaluate}. Results stay in memory on this page.",
  },
  status: {
    noKey: "No API key",
  },
  ...
  run: {
    ...
    fixInAuthor: "Fix them in Edit mode",
  ...
  batch: {
    ...
    hint: "Run the current rubric on 100 bundled students. Each row is one Jev call with all {count} questions. Results live in this page only — refresh clears them.",
    ...
    costHint: "Cost is input tokens × $0.042 / MTok. Output is free.",
    ...
    emptyCell: "—",
    noneFired: "None",
    costDetail: "{cost} · {in} in / {out} out tokens",
    tableCaption: "One hundred students with status, profile outcomes, latency, and cost",
  },
  answers: {
  ...
  profile: {
    ...
    noConditions: "No conditions defined. Add some in Edit mode.",
...
export const zhCN: DeepString<typeof en> = {
  ...
  app: {
    ...
    batchFooter: "每一行都是一次经过 {evaluate}、带上整份量表的 Jev {systemone} 调用。结果只留在本页内存中。",
  },
  status: {
    noKey: "未配置 API 密钥",
  },
  ...
  batch: {
    title: "批量分类",
    hint: "用当前量表评测 100 名内置学生。每一行是一次包含全部 {count} 道题的 Jev 调用。结果只留在本页，刷新即清空。",
    ...
    avgMs: "平均 {ms} ms",
    totalCost: "合计 {cost}",
    costHint: "费用按输入 tokens × $0.042 / MTok 计算，输出免费。",
    ...
    emptyCell: "—",
    noneFired: "无",
    costDetail: "{cost} · 输入 {in} / 输出 {out} tokens",
    tableCaption: "一百名学生的状态、画像结果、耗时与费用",
  },
  answers: {
    noAnswer: "这道题没有返回答案。",
    pYes: "P(是)",
