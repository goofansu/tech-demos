...
export const en = {
  ...
  app: {
    ...
    author: "Author",
    batch: "Batch",
    batchSubtitle: "{count} records · {limit} at a time · TypeSafe Jev only",
    language: "Language",
    footer:
    ...
    batchFooter:
      "Each row is one Jev {systemone} call through {evaluate}. Results stay in memory on this page.",
  },
  status: {
    noKey: "No API key",
  },
  ...
  batch: {
    ...
    hint: "One fixed Choice question on 100 bundled student texts. Each row is its own Jev call. Results live in this page only — refresh clears them.",
    questionTitle: "Fixed question",
    questionLabel: "Dominant learning signal",
    questionHelp: "Read {state} only. Pick the single best label. If two signals are equally strong, choose mixed.",
    concurrency: "Up to {n} requests at once",
    ...
    avgMs: "avg {ms} ms",
    noPersist: "Results start empty and are never written to localStorage or the server.",
    colIndex: "#",
    colName: "Student",
    colPreview: "Text",
    colStatus: "Status",
    ...
    optionReflection: "Reflection",
    optionMixed: "Mixed",
    confidencePct: "{pct}%",
    emptyCell: "—",
    resultDetail: "{label} · confidence {pct}%",
    tableCaption: "One hundred student texts and their Jev classifications",
  },
  answers: {
...
export const zhCN: DeepString<typeof en> = {
  ...
  app: {
    ...
    author: "编写",
    batch: "批量",
    batchSubtitle: "{count} 条记录 · 每次 {limit} 路并发 · 仅使用 TypeSafe Jev",
    language: "语言",
    footer:
      "Jev 在一次 {systemone} 请求中回答全部题目；阈值与条件在代码中判定。API 密钥仅留在服务器的 {evaluate} 接口之后。",
    batchFooter: "每一行都是一次经过 {evaluate} 的 Jev {systemone} 调用。结果只留在本页内存中。",
  },
  status: {
    noKey: "未配置 API 密钥",
  },
  ...
  batch: {
    title: "批量分类",
    hint: "用一道固定的 Choice 题对 100 条内置学生文本逐条调用 Jev。结果只留在本页，刷新即清空。",
    questionTitle: "固定题目",
    questionLabel: "主导学习信号",
    questionHelp: "只根据 {state} 判断。选一个最贴切的标签。若两个信号差不多强，选 mixed。",
    concurrency: "最多同时 {n} 个请求",
    classifyAll: "全部分类",
    classifying: "正在分类…",
    stop: "停止",
    reset: "重置结果",
    ...
    avgMs: "平均 {ms} ms",
    noPersist: "结果从空白开始，不会写入 localStorage 或服务器。",
    colIndex: "#",
    colName: "学生",
    colPreview: "文本",
    colStatus: "状态",
    ...
    optionSupport: "求助",
    optionExtension: "延伸",
    optionCollaboration: "协作",
    optionReflection: "反思",
    optionMixed: "混合",
    confidencePct: "{pct}%",
    emptyCell: "—",
    resultDetail: "{label} · 置信度 {pct}%",
    tableCaption: "一百条学生文本及其 Jev 分类结果",
  },
  answers: {
    noAnswer: "这道题没有返回答案。",
    pYes: "P(是)",
