  ...
  return (
    <Card>
      ...
      <CardContent className="flex flex-col gap-3">
        ...
        {judgment.never_not_met && outcome.role === "field" ? (
          <p className="text-xs text-muted-foreground">
            {outcome.neverNotMetProtected
              ? "never_not_met replaced Not Met with Needs Review."
              : "Protected judgment — cannot be Not Met."}
          </p>
        ) : null}
        ...
        {answer?.type === "noul" ? <NoulBody noul={answer.noul} /> : null}
        {answer?.type === "choice" ? (
          <ChoiceBody
...
function NoulBody({ noul }: { noul: number }) {
  return (
    <div className="flex flex-col gap-2">
      ...
      <Meter value={noul} tone="noul" label="Probability yes" />
    </div>
  );
}
  ...
  return (
    <details className="rounded-lg border bg-muted/40">
      <summary className="cursor-pointer px-3 py-2 text-xs font-medium select-none">
        {choice} · confidence {confidence.toFixed(2)} · show distribution
      </summary>
      <ul className="flex flex-col gap-2 border-t px-3 py-3">
        {rows.map(([key, p]) => (
          <li key={key} className="flex flex-col gap-1">
            ...
            <Meter value={p} tone={key === choice ? "choice" : "muted"} label={`Probability ${key}`} />
          </li>
        ))}
      </ul>
    </details>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-baseline justify-between gap-3">
        <div>
          <span className="text-xs text-muted-foreground">Weighted score (not rounded)</span>
          ...
          {ordered.map((lvl) => {
            ...
            return (
              <li key={lvl} className="flex flex-col gap-1">
                ...
                <Meter value={p} tone="score" label={`Level ${lvl} probability`} />
              </li>
            );
          })}
        </ol>
